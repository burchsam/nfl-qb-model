...
next_year_df1 = nfl_qbs_season |>
  ...
  ungroup() |> 
  select(-c(player, year, draft_year, qbr_pct_tot, mean_epa_tot, cpoe_tot, ptsr_tot))

nfl_qbs_season |> filter(player == "Caleb Williams") |> arrange(year)
...
ggplot(cor_tbl, aes(x = abs(next_qbr_pct), y = reorder(names, abs(next_qbr_pct)))) +
  ...
  labs(
    ...
    y = "Predictors",
    x = "Correlation",
    caption = "By: Sam Burch  |  Data: nflfastR & pff (2006-2025)",
    fill = element_blank()
  ) +
...
## Constant Variance
bptest(m1)
### With the p-value = .01 < .05 = $\alpha$, we reject the null and conclude the constant variance assumption is not satisfied.

## Normaility, n < 50
shapiro.test(m1$residuals)
### With the p-value = .25 > .05 = $\alpha$, we fail to reject the null and conclude the normality assumption is satisfied.
...
all_qbs_last = nfl_qbs_season |> 
  filter(!is.na(qbr_pct)) |> 
  arrange(player, year) |> 
  group_by(player) |> 
  summarise(last_season = last(year),
            .groups = "drop")
  
  

qbs_last = nfl_qbs_season |> 
  left_join(all_qbs_last, by = "player") |> 
  filter(!is.na(qbr_pct)) |>
  group_by(player) |> 
  arrange(year) |> 
  mutate(qbr_pct_avg = cummean(qbr_pct),
         ...
         plays_tot = cumsum(dropbacks)) |> 
  ungroup() |> 
  filter(year == last_season)

qbs_last |> colnames()

pred_2026 = predict(lasso_model, newx = as.matrix(qbs_last |> select(-c(player, season, 
                                                               first_year, year, draft_year, qbr_pct_tot, mean_epa_tot, 
                                                               cpoe_tot, ptsr_tot))), s = cv_lasso$lambda.min)
pred_2026 |> as_vector()
...
qbs_2026 = qbs_last |>
  mutate(pred_26 = pred_2026) |> 
  select(player, pred_26, qbr_pct_tot, mean_epa_tot, grade_avg, everything()) |> 
  arrange(-pred_26) |> 
  left_join(qb_teams, by = c("player" = "full_name")) |> 
  filter(!is.na(team))

# write.csv(qbs_2025, file = "qb-projections-2025.csv")

  
# Projections vs. Career
ggplot(qbs_2026 |>  filter(!is.na(qbr_pct_tot)), aes(x = (qbr_pct_tot)*100, y = pred_26)) +
  ...
  labs(
    title = "Career QB Play vs. 2026 Projections",
    subtitle = "400+ career dropbacks  |  percentile = 50-50 compositie of epa/play & pff grade  |  projections use lasso model",
    caption = "By: Sam Burch  |  Data @nflfastR & @pff (since 2006)",
    x = "Career Percentile",
    y = "2026 Percentile Projections"
  ) +
  # nflplotR::scale_fill_nfl(alpha = .8) +
  # nflplotR::scale_color_nfl(type = "primary") +
  nflplotR::geom_mean_lines(aes(x0 = (qbr_pct_tot)*100, y0 = pred_26, alpha = .8)) +
  theme(
...
ggsave("nfl-qbs-26-career.png", width = 16, height = 9, units = "cm")
